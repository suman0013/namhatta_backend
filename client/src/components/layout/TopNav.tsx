...
export default function TopNav() {
  ...
  return (
    <nav className="flex justify-center px-2 overflow-x-auto">
        ...
        {navigationItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}>
              <div
                className={cn(
                  "flex items-center px-2 xl:px-3 py-1.5 text-xs xl:text-sm font-medium rounded-lg transition-all duration-200 relative whitespace-nowrap",
                  isActive
              ...
              >
                <Icon className="w-3 xl:w-4 h-3 xl:h-4 mr-1 xl:mr-2 flex-shrink-0" />
                <span className="hidden xl:inline text-sm">{item.label}</span>
                <span className="xl:hidden text-xs">{item.shortLabel}</span>
                {item.badge && (
                  <Badge
                    variant={isActive ? "default" : "secondary"}
                    className="ml-1 xl:ml-2 h-4 text-xs flex-shrink-0 px-1"
                  >
                    {item.badge}
                  </Badge>
                )}
